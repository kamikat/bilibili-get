...
var downloadFiles = function* (taskInfo, { dryRun, print, outputPattern, outputFormat, downloadOptions }) {
  ...
  for (var i = 0, N = taskInfo.durl.length; i++ != N; segmentFiles[i-1] = yield (function* ({ order, url, size, backup_url }) {
    ...
    var downloadCommand = `aria2c --no-conf --console-log-level error --file-allocation none -c -d "${path.dirname(filePath)}" -o "${path.basename(filePath)}" --referer "${taskInfo.url}" ${downloadOptions||''} "${url}"`;
