...
module.exports = (url, { dryRun, listFormats, silent, quality, cookie, output, outputFormat }) => function* () {
  ...
  for (var i = 0, N = taskList.length; i++ != N; yield (function* (taskInfo) {
    ...
    if (!listFormats) {
      ...
      if (yield fs.exists(outputPath)) {
        ...
        var segmentFiles = yield downloadFiles(taskInfo, { print, dryRun, outputDir: path.dirname(outputPath) });
